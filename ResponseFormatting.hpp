...
class ResponseFormatting
{
	public:
		...
		static std::string	parse_cgi_headers(std::string http_version, int content_length, int &status_code,
						std::deque<std::string> status_infos);
		static std::string	parse_body(std::string file_path, int const &status_code);
		static std::string	format_response(HttpRequest &request, int &status_code, std::string &file_path, Config &config);
		static std::deque<std::string>	get_status_infos(int status_code, std::string &file_path, std::string &error_path);
};
